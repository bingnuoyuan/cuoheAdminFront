<template>
  <div class="add-page">
    <el-form ref="order" :model="order" :rules="rules">
      ...
      <el-form-item label="订单号" required prop="orderId">
      ...
      <el-form-item label="合同信息" required >
        <div class="listInfo">
          <div v-for="item in order.contractInfo">
            <el-image class="el-upload--picture-card" :src="item.download_url" fit="fill"></el-image>
          </div>
        </div>
        <el-upload
    ...
    <el-dialog title="提货信息" :visible.sync="dialogDeliverVisible">
    ...
    <!--产品信息编辑框-->
    <el-dialog title="产品信息" :visible.sync="dialogEditVisible">
    ...
    <!--编辑提货信息对话框-->
    <el-dialog title="提货信息" :visible.sync="dialogDeliverEditVisible">
...
<script>
import { fetchById, update } from '@/api/order'
...
export default {
  ...
  data() {
    return {
      ...
      rules:{
        ...
        product: [
            { required: true, message: '请填写产品类型', trigger: 'change' }
        ],
        contract: [
  ...
  methods: {
    ...
    doEditDeliver(){
      console.log("this.order.deliverInfo", this.order.deliverInfo)
    ...
    doSave(){
      ...
      this.productInfo.push(this.productAdd);
    ...
    onSubmit() {
      let that = this;
      console.log('that.order', that.news)
      update(that.order).then((res)=>{
        console.log(res)
